...
powershell -STA -NoProfile -Command "Add-Type -AssemblyName System.windows.forms; $f = New-Object System.Windows.Forms.FolderBrowserDialog; $f.Description = 'AgentSmith를 설치할 최상위 폴더를 선택하세요 (이 폴더 안에 agentsmith 폴더가 생성됩니다)'; $f.ShowNewFolderButton = $true; $f.RootFolder = 'MyComputer'; $form = New-Object System.Windows.Forms.Form; $form.TopMost = $true; if($f.ShowDialog($form) -eq 'OK'){ [IO.File]::WriteAllText($env:TEMP + '\agentsmith_install_dir.txt', $f.SelectedPath) } else { [IO.File]::WriteAllText($env:TEMP + '\agentsmith_install_dir.txt', 'CANCEL') }"
